...
const robotStats = `Robot is busy`

const sendMarsTemperature = (msgOK, msgERR) =>  setTimeout(() => {
  const chance = Math.floor(Math.random() * 101) >= 0.6;
  chance ? msgOK(getMarsTemperature()) : msgERR(handleError(robotStats)), messageDelay()});
